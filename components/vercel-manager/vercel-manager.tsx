...
function ProjectWorkspace({
  ...
  const handleDelete = async (project: VercelProject) => {
    if (
      !confirm(`Delete project "${project.name}"? This cannot be undone.`)
    )
      return
    try {
          ...
          <ul className="divide-y divide-white/5">
            {visible.map((project) => {
              ...
              return (
                <li
                  key={project.id}
                  className="group flex items-center gap-2 px-3 py-2.5 hover:bg-white/5 transition-colors"
                >
                  <CircleDotIcon
                    className={cn(
                      'w-3.5 h-3.5 flex-shrink-0',
                      ready ? 'text-emerald-500/80' : 'text-slate-600'
                    )}
                  />
